...
EST=function(n, n_1, p){
  ...
  g_x=kappa*mean((1-S)*Mmu_11/mu_01*(Y))
  iptw_x=kappa*mean(S*Mtau*Mmu_00/mu_01*A/q*Y)
  
  results=c(est_all_v,g_v,iptw_v,
            est_11,g_11,iptw_11,
            est_00,g_00,iptw_00,
            est_all_x,g_x,iptw_x)
  return(results)
...
k=5000
# 
vec_5500_500=matrix(NA,nrow=k,ncol=12)
...
# 
# #### plots
# bias_all_v=bias_5500_500[1:3];bias_11=bias_5500_500[4:6];bias_00=bias_5500_500[7:9];bias_all_x=bias_5500_500[10:12]
# sd_all_v=sim_sd_5500_500[1:3];sd_11=sim_sd_5500_500[4:6];sd_00=sim_sd_5500_500[7:9];sd_all_x=sim_sd_5500_500[10:12]
# # bias=c(bias_all_v,bias_11,bias_00,bias_all_x)
...
#       main="Comparison of different model performance, n_1=500", xaxt="n")
...
# for (i in 1:3){
#   points(0.8+0.1*i,bias_all_v[i],pch=(i-1)*10, cex=1.5)
#   segments(0.8+0.1*i, bias_all_v[i]-sd_all_v[i], 0.8+0.1*i, bias_all_v[i]+sd_all_v[i], lwd=3, lty = 3 )
# }
# for (i in 1:3){
#   points(1.8+0.1*i,bias_11[i],pch=(i-1)*10, cex=1.5)
#   segments(1.8+0.1*i, bias_11[i]-sd_11[i], 1.8+0.1*i, bias_11[i]+sd_11[i], lwd=3, lty = 3 )
# }
# for (i in 1:3){
#   points(2.8+0.1*i,bias_00[i],pch=(i-1)*10, cex=1.5)
#   segments(2.8+0.1*i, bias_00[i]-sd_00[i], 2.8+0.1*i, bias_00[i]+sd_00[i], lwd=3, lty = 3 )
# }
# for (i in 1:3){
#   points(3.8+0.1*i,bias_all_x[i],pch=(i-1)*10, cex=1.5)
#   segments(3.8+0.1*i, bias_all_x[i]-sd_all_x[i], 3.8+0.1*i, bias_all_x[i]+sd_all_x[i], lwd=3, lty = 3 )
# }
# abline(h=0)
# 
# legend(0.5, -0.15, c('Doubly robust', 'Regression', 'IPTW'),  horiz = F, bty = 'n', cex=1.2,pch=c(8,9,10))
# 
######
vec_6000_1000=matrix(NA,nrow=k,ncol=12)
...
# 
# #### plots
# bias_all_v=bias_6000_1000[1:3];bias_11=bias_6000_1000[4:6];bias_00=bias_6000_1000[7:9];bias_all_x=bias_6000_1000[10:12]
# sd_all_v=sim_sd_6000_1000[1:3];sd_11=sim_sd_6000_1000[4:6];sd_00=sim_sd_6000_1000[7:9];sd_all_x=sim_sd_6000_1000[10:12]
# # bias=c(bias_all_v,bias_11,bias_00,bias_all_x)
# plot( c(1,4), las = 2, xlim =c(0.6, 4.4), ylim = c(-0.2, 0.2),  type="n",
#       xlab=TeX(r'(Correct Model(s))',bold=TRUE), ylab=TeX(r'(Bias$\pm$SD)', bold=TRUE), 
#       main="Comparison of different model performance, n_1=1000", xaxt="n")
# axis(1, at=1:4, labels=c(TeX(r'($\{\mu_{1, 1}(X), \mu_{0}(X), \tau(X)\}$)', bold=FALSE),TeX(r'($\{\mu_{1, 1}(X)\}$)', bold=FALSE),TeX(r'($\{\mu_{0}(X), \tau(X)\}$)', bold=FALSE),TeX(r'(None)', bold=FALSE)) )
# for (i in 1:3){
#   points(0.8+0.1*i,bias_all_v[i],pch=(i-1)*10, cex=1.5)
#   segments(0.8+0.1*i, bias_all_v[i]-sd_all_v[i], 0.8+0.1*i, bias_all_v[i]+sd_all_v[i], lwd=3, lty = 3 )
...
# legend(0.5, -0.15, c('Doubly robust', 'Regression', 'IPTW'),  horiz = F, bty = 'n', cex=1.2,pch=c(8,9,10))
# # ########
vec_10000_5000=matrix(NA,nrow=k,ncol=12)
...
# 
# bias_all_v=bias_10000_5000[1:3];bias_11=bias_10000_5000[4:6];bias_00=bias_10000_5000[7:9];bias_all_x=bias_10000_5000[10:12]
# sd_all_v=sim_sd_10000_5000[1:3];sd_11=sim_sd_10000_5000[4:6];sd_00=sim_sd_10000_5000[7:9];sd_all_x=sim_sd_10000_5000[10:12]
# # bias=c(bias_all_v,bias_11,bias_00,bias_all_x)
# plot( c(1,4), las = 2, xlim =c(0.6, 4.4), ylim = c(-0.2, 0.2),  type="n",
#       xlab=TeX(r'(Correct Model(s))',bold=TRUE), ylab=TeX(r'(Bias$\pm$SD)', bold=TRUE), 
#       main="Comparison of different model performance, n_1=5000", xaxt="n")
# axis(1, at=1:4, labels=c(TeX(r'($\{\mu_{1, 1}(X), \mu_{0}(X), \tau(X)\}$)', bold=FALSE),TeX(r'($\{\mu_{1, 1}(X)\}$)', bold=FALSE),TeX(r'($\{\mu_{0}(X), \tau(X)\}$)', bold=FALSE),TeX(r'(None)', bold=FALSE)) )
# for (i in 1:3){
#   points(0.8+0.1*i,bias_all_v[i],pch=(i-1)*10, cex=1.5)
#   segments(0.8+0.1*i, bias_all_v[i]-sd_all_v[i], 0.8+0.1*i, bias_all_v[i]+sd_all_v[i], lwd=3, lty = 3 )
...
legend(0.5,-0.05, c(TeX(r'($\widehat{\alpha_{1}}$)', bold=FALSE),TeX(r'($\widehat{\alpha_{1}^G}$)', bold=FALSE),TeX(r'($\widehat{\alpha_{1}^W}$)', bold=FALSE)),  horiz = F, bty = 'n', cex=1.4,pch=c(0,10,20))
